...
local function SetupNativeSettings()
    ...
    for _, v in pairs(PlayerLEDModeList) do
        local data = v({}, true)
        playerLEDList[i] = data.LEDName
        i = i + 1
    end

    NS.addSelectorString(extraSettings, GetText('Mod-DualSense-NS-PlayerLED-Mode'), GetText('Mod-DualSense-NS-PlayerLED-Mode-Descr') .. ' ' .. GetText('Mod-DualSense-NS-OnlySteamDSX'), playerLEDList, config.playerLEDValue, 2, function(v)

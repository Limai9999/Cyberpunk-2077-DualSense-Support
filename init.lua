...
registerForEvent('onUpdate', function(delta)
    ...
    if (isMainMenu) then
        ...
        if not (config.playerLED) then data.playerLED = '(False)(False)(False)(False)(False)' end
    ...
    local pLEDlist = {}
    local i = 1
    for pLEDmodeName in pairs(PlayerLEDModeList) do
        pLEDlist[i] = pLEDmodeName
        i = i + 1
    end
    ...
    if (veh ~= nil and veh:IsPlayerDriver()) then
        ...
        if (vehType.data.hasOwnMode) then
            ...
            if not (data.overwritePlayerLED) then data.playerLED = prevPlayerLED end
